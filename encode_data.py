...
def encode_string_tokens(preprocessed_train_embs, train_embs, num_groups, num_clusters, max_iter): # genterate encoded string tokens
...
def save_results(num_groups, num_clusters, kmeans, encoded_string_list): #### SAVE KMEANS MODEL AND
...
def run_encode_data(nums_groups, nums_clusters, train_embs, max_iter):
    for num_groups in nums_groups:
        for num_clusters in nums_clusters:
            ...
            preprocessed_train_embs = preprocess_train_embs_vectors(train_embs,
                                                                    num_groups)
            kmeans, encoded_string_list = encode_string_tokens(preprocessed_train_embs,
                                                               train_embs,
                                                               num_groups,
                                                               num_clusters,
                                                               max_iter)
            save_results(num_groups,
                         num_clusters,
                         kmeans,
                         encoded_string_list)
...
def encode_main():
    ...
    run_encode_data(nums_groups, nums_clusters, train_embs, max_iter)
